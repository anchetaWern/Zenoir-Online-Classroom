...
class assignments_model extends ci_Model{
		...
		function view($assignment_id){//view a single assignment
		...
		function reply_details($assignment_id){//get the details needed when replying to an assignment: title of the assignment you're replying to and assignment_id
			...
			if($assignment->num_rows() > 0){
				...
				$as_title = $row->as_title;
				$as_body  = $row->as_body;
				$assignment_details = array('as_id'=>$assignment_id, 'as_title'=>$as_title, 'as_body'=>$as_body);
			}
			return $assignment_details;
		}
		...
		function list_replies($assignment_id){//loads the replies to a specific assignment
		...
		function view_reply($reply_id){//view a specific reply to an assignment
		...
		function response_id($assignment_id){
